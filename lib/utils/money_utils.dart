...
class MoneyUtil {
  ...
  /// 分 转 元, format格式输出.
  static String changeF2Y(num amount,
  ...
  static String changeF2YWithUnit(num amount,
      {MoneyFormat format = MoneyFormat.NORMAL,
        MoneyUnit unit = MoneyUnit.YUAN}) {
  ...
  /// 拼接单位.
  static String numToMoney(num value) {
    String valueStr = value.toString();
    if(!valueStr.contains('.')){
      return '$value.00';
    }else{
      return NumUtil().formatNum(value.toDouble(),2);
    }
  }
}